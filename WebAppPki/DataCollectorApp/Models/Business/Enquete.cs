...
public class Enquete : Persist
{
    ...
    public override bool fnUpdate()
    {
        ...
        switch (RetVal)
        {
            case 0:
                ...
                if (IsNew)
                    EnqueteID = (int)db.obtenirParametre("@ID");
    ...
    public override bool fnDelete()
    {
        ...
        db.ajouterParametreEntree("@ID", 32, SqlDbType.BigInt, EnqueteID);
    ...
    private void mapFromDataReaderToObject(IDataReader reader, Enquete mClass)
    {
        ...
        if (reader != null)
        {
            if (!Convert.IsDBNull(reader["ID"]))
                mClass.EnqueteID = (long)reader["ID"];
            ...
            if (!Convert.IsDBNull(reader["DateFin"]))
                mClass.DateFin = DateTime.Parse(reader["DateFin"].ToString());
